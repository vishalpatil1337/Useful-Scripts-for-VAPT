...
class DNSVulnScanner(BaseScanner):
    def verify(self, vuln):
        ip, port = vuln["ip"], "53"  # Default DNS port
        vuln_name = vuln["vulnerability"].lower()
        output_dir = create_poc_folder("Verified", "dns", vuln)
        nmap_output = os.path.join(output_dir, "nmap_scan")

        try:
            script = "dns-zone-transfer" if "zone" in vuln_name else "dns-cache-snooping"
            ...
            logger.error(f"\033[91mDNS scan failed for {ip}:{port}: {e}\033[0m")
