# scanners/nginx_vuln_scanner.py
# Nginx vulnerability scanning
...
class NginxVulnScanner(BaseScanner):
    def verify(self, vuln):
        ...
        output_dir = create_poc_folder("Verified", "nginx", vuln)
        nmap_output = os.path.join(output_dir, "nmap_scan")

        try:
            script = "http-default-pages" if "default" in vuln_name else "http-vuln-nginx"
            ...
            logger.error(f"\033[91mNginx scan failed for {ip}:{port}: {e}\033[0m")
