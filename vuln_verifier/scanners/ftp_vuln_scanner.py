...
class FTPVulnScanner(BaseScanner):
    def verify(self, vuln):
        ip, port = vuln["ip"], vuln["port"]
        vuln_name = vuln["vulnerability"].lower()
        output_dir = create_poc_folder("Verified", "ftp", vuln)
        nmap_output = os.path.join(output_dir, "nmap_scan")

        try:
            script = "ftp-anon" if "anonymous" in vuln_name else "ftp-brute"
            ...
            logger.error(f"\033[91mFTP scan failed for {ip}:{port}: {e}\033[0m")
