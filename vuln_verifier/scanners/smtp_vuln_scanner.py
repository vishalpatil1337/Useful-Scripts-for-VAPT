# scanners/smtp_vuln_scanner.py
# SMTP vulnerability scanning
...
class SMTPVulnScanner(BaseScanner):
    def verify(self, vuln):
        ...
        try:
            script = "smtp-open-relay" if "relay" in vuln_name else "smtp-enum-users"
            ...
            logger.error(f"\033[91mSMTP scan failed for {ip}:{port}: {e}\033[0m")
