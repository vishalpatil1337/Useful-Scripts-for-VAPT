...
class SMBVulnScanner(BaseScanner):
    def verify(self, vuln):
        ip, port = vuln["ip"], "445"  # Default SMB port
        vuln_name = vuln["vulnerability"].lower()
        output_dir = create_poc_folder("Verified", "smb", vuln)
        nmap_output = os.path.join(output_dir, "nmap_scan")

        try:
            script = "smb-vuln-ms17-010" if "eternalblue" in vuln_name else "smb-vuln-*"
            ...
            logger.error(f"\033[91mSMB scan failed for {ip}:{port}: {e}\033[0m")
